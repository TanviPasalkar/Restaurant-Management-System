...
def BookTableView(request):
    if request.method=='POST':
        ...
        if name != '' and len(phone_number) == 10 and email != '' and total_person != 0 and booking_data != '':
            data = BookTable(Name=name, Phone_number=phone_number, Email=email, Total_person=total_person, Booking_date=booking_data)
            data.save()
...
def FeedbackView(request):
    if request.method=='POST':
        ...
        if name != '' and description != '' and rating != 0 and feedback_image != '':
            data = Feedback(User_name=name, Description=description, Rating=rating, Image=feedback_image)
            data.save()
